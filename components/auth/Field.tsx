...
const Field = ({text, type, setter}: FieldProps) => {
  ...
  return (
    ...
    <div className='w-4/6 mx-auto flex flex-col relative'>
        <input id={`${type}-input`} required type={type} placeholder={text === 'Email Address' ? 'Email' : 'Password'} className='bg-white rounded-md h-14 border-4 border-white-300 w-full pl-3 focus:outline-none focus:border-focus invalid:border-error peer
        valid:border-success' {...(type === 'password' ? {pattern: "^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$"} : {})}
        ...
        <div className='absolute md:-bottom-6 -bottom-8 left-0 text-white-80 text-xs peer-invalid:text-error peer-valid:text-success peer-focus:text-white-80'>Must be at least 8 characters and contain at least 1 digit, 1 uppercase letter, and 1 special character.</div>}
